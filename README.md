...
Planning remains available without an account. Server-side saved tours use optional accounts. Email verification is required in production, while local development activates a newly registered account immediately and sends no verification email. Set `AUTH_EMAIL_VERIFICATION_REQUIRED=true` to test the production flow locally. Login, registration, email verification, and password reset use Django sessions and time-limited one-time links. Authentication and public compute endpoints are rate-limited in Django's cache. Identical elevation profiles and location searches are cached to reduce repeated upstream requests. For more than one backend process, configure Django's cache with a shared backend before scaling out.
...
AUTH_VERIFICATION_EMAIL_RATE_LIMIT=3
AUTH_VERIFICATION_EMAIL_RATE_WINDOW_SECONDS=3600
MONITORING_REPORT_RECIPIENTS=admin@your-domain.example
MONITORING_REPORT_LOOKBACK_HOURS=24
DATABASE_BACKUP_DIR=/data/app/backups
