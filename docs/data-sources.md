...
For routes calculated by GraphHopper, `road_environment=BRIDGE` path details identify bridge ranges. Imported GPX tracks are matched against OSM ways tagged `bridge=*` in the local Switzerland extract. Elevation samples within matched ranges are interpolated between the surrounding terrain samples before smoothing, gradient, ascent/descent, and time calculations. This avoids treating the terrain or water below a mapped bridge as the route elevation. GPX bridge detection depends on OSM mapping and the track's positional accuracy; it never re-routes the imported geometry.
